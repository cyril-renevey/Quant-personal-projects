...
class Market:
    ...
    def __init__(self,stocks_list,start_date,end_date,include_options=False):
        ...
        column_values=['Adj Close','Volume']
        ...
        self.rate = \
                yf.download(tickers='^IRX',start=start_date,end=end_date+dt.timedelta(days=1),interval='1d')[['Adj Close']]/100
...
class Position:
    ...
    def value(self):
        ...
        call_value = sum([sum([j[2]*BS_call(a.stocks[i].Close[self.time],\
                    ...
                    a.options[i].implied_volatility[self.time]) for j in self.call_pos[i]]) for i in a.tickers])
        put_value = sum([sum([j[2]*BS_put(a.stocks[i].Close[self.time],\
                    j[0], j[1]/365, a.rate.Close[self.time], \
                    a.options[i].implied_volatility[self.time]) for j in self.put_pos[i]]) for i in a.tickers])
